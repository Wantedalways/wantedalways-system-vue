  ...
  <div class="depart-container">
    <div class="depart-wrapper">
      <div class="depart-search-wrapper">
        <el-input :prefix-icon="Search" placeholder="搜索部门、用户" clearable/>
        <el-button :icon="Plus" circle class="add-button" @click="handleDepartAddButtonClick"/>
      </div>
          ...
          <el-scrollbar class="depart-scrollbar">
            ...
            >
              <template #default="{data}">
                <div class="tree-node" @mouseenter="handleNodeMouseEnter(data.id)" @mouseleave="handleNodeMouseLeave">
                  ...
                  >
                    ...
                    <template #default>
                      <div class="depart-popover-content">
                        <div class="depart-popover-item" @click="openEditDepartDialog(data)">修改</div>
                        <div class="depart-popover-item" @click="handleDeleteDepart(data)">删除</div>
                        <div class="depart-popover-item" @click="handleAddChildDepart(data.id)">添加子部门</div>
                      </div>
                    </template>
                  </el-popover>
                </div>
  ...
  <el-dialog v-model="departAddDialogVisible" title="新增部门" width="550">
    ...
    >
      <el-form-item label="上级部门" prop="parentId">
        <el-tree-select :data="departTreeData" check-strictly v-model="departAddParams.parentId" filterable clearable
                        class="depart-form-item"/>
      </el-form-item>
      <el-form-item label="部门名称" prop="departName">
        <el-input v-model="departAddParams.departName" clearable class="depart-form-item"/>
      </el-form-item>
      <el-form-item label="企业微信编码" prop="workWxId">
        <el-input v-model="departAddParams.workWxId" clearable class="depart-form-item"/>
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button class="depart-form-button" @click="departAddDialogVisible = false">取消</el-button>
      <el-button class="depart-form-button" type="primary" @click="handleAddDepart">确认</el-button>
    </template>
  </el-dialog>

  <el-dialog v-model="departEditDialogVisible" title="修改部门" width="550">
    <el-form :model="departEditParams"
             label-position="right"
             label-width="auto"
             class="depart-form"
             hide-required-asterisk
    ...
    >
      <el-form-item label="上级部门" prop="parentId">
        <el-tree-select :data="editFilterDepartTree"
                        check-strictly
      ...
      <el-form-item label="部门名称" prop="departName">
        <el-input v-model="departEditParams.departName" clearable class="depart-form-item"/>
      </el-form-item>
      <el-form-item label="企业微信编码" prop="workWxId">
        <el-input v-model="departEditParams.workWxId" clearable class="depart-form-item"/>
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button class="depart-form-button" @click="departEditDialogVisible = false">取消</el-button>
      <el-button class="depart-form-button" type="primary" @click="handleEditDepart">确认</el-button>
    </template>
  </el-dialog>
</template>
...
import {Management, MoreFilled, Plus, Search} from "@element-plus/icons-vue";
...
async function validateName(rule: any, value: any, callback: any) {
  if (value === '') {
    ...
    if (editDepartValidateParams.departName === '' || editDepartValidateParams.departName !== departEditParams.value.departName) {
...
async function validateWorkWxId(rule: any, value: any, callback: any) {
  if (value === '') {
    ...
    if (editDepartValidateParams.workWxId === '' || editDepartValidateParams.workWxId !== departEditParams.value.workWxId) {
  ...
  const markDisabled = (nodes, parentDisabled = false) => {
    return nodes.map(node => {
      ...
      return {
        ...node,
        disabled: isDisabled,
        children: node.children ? markDisabled(node.children, isDisabled) : undefined
      }
    })
  }
